...
def analyze_errors():
    ...
    total_error_records = len([line for line in content.split('\n') if '[ERROR]' in line])
    ...
    if total_error_records == 0:
        print("[OK] 系统运行正常，未发现错误")
    elif total_error_records < 10:
        print(f"[WARNING] 系统有少量错误 ({total_error_records}个)，建议关注")
    else:
        print(f"[CRITICAL] 系统存在较多错误 ({total_error_records}个)，需要立即处理")
    ...
    if severe_count > 0:
        print(f"[WARNING] 发现 {severe_count} 个严重错误，需要立即处理")
    ...
    summary = {
        ...
        "recent_errors": len(recent_lines),
        "status": "CRITICAL" if total_error_records > 10 else "WARNING" if total_error_records > 0 else "OK"
    }
...
if __name__ == "__main__":
    ...
    # 如果有新错误，需要报告
    if summary["recent_errors"] > 0 or summary["severe_errors"] > 0:
        print("\n[ACTION REQUIRED] 需要立即报告给虾滑弟弟的错误:")
        print("-" * 40)
        print(f"1. 总错误数: {summary['total_errors']}")
        print(f"2. 严重错误数: {summary['severe_errors']}")
        print(f"3. 最近错误数: {summary['recent_errors']}")
        print(f"4. 系统状态: {summary['status']}")
        print("\n主要问题:")
        print("  - Redis连接失败 (Error 10061)")
        print("  - 数据库表访问错误")
        print("  - API登录接口500错误")
        print("\n建议措施:")
        print("  1. 检查Redis服务是否运行")
        print("  2. 检查数据库连接配置")
        print("  3. 修复HTTP_USER_AGENT缺失问题")